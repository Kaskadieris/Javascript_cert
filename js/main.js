...
function get_change(to_return, currency) {
  ...
  if (to_return >= nominal) {
    ...
    let sum = 0.0;
    while (
      roundNumber(to_return, 2) >= roundNumber(nominal, 2) &&
      amount != 0
    ) {
      amount--;
...
projects[0].addEventListener("submit", (e) => {
  ...
  if (palindrome(value)) {
    if (value == "") {
      input_form.getElementsByTagName("p")[0].innerHTML =
        "Please input a word!!!";
    } else {
...
projects[3].addEventListener("submit", (e) => {
  ...
  if (value == "") {
    input_form.getElementsByTagName("p")[0].innerHTML =
      "Please input US number!!!";
  } else {
    if (telephoneCheck(value) == false) {
      input_form.getElementsByTagName("p")[0].innerHTML = "INVALID US NUMBER";
    } else {
...
projects[4].addEventListener("submit", (e) => {
  ...
  if (price == "" || cash == "") {
    ...
    input_form.getElementsByTagName(
      "p"
    )[0].innerHTML = `STATUS:${result.status}`;
